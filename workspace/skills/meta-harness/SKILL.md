---
name: meta-harness
description: 자기 자신(이 하네스)을 격리된 복사본으로 헤드리스 실행해 실행기록·산출물을 확인하고, 시스템 프롬프트·도구코드·스킬을 격리 환경에서 수정해 다시 실행한 뒤, 두 결과를 비교해 확실히 더 나을 때만 본체에 반영(promote)하고 애매하면 무승부로 baseline 을 유지하는 자기개선 절차. "meta-harness", "자기개선", "self-improve", "이 질의로 나를 개선", "프롬프트/도구/스킬 A/B 로 실험" 같은 요청에 사용.
---
...
5. 두 실행을 **비교**한다.
6. v1 이 **확실히 더 나을 때만** 사용자 확인 후 본체에 **promote**. 우위가 애매하면
   **무승부로 보고 baseline(본체)을 유지**한다(필요 시 `v2` 로 반복).
...
`final_answer.md`, 산출물을 직접 읽고 **성공기준에 비추어 우열을 판단**하라.

**판정 원칙 — 확실한 우위일 때만 승리, 애매하면 무승부다(기본값=무승부).**
promote 는 본체를 바꾸는 되돌리기 비싼 행동이므로, 판정의 기본값은 "무승부(=반영 안 함)"로
두고, **명백하고 결정적인 우위가 증명될 때만** 승리로 올린다. 다음은 승리가 아니라 무승부다:
...
즉 판정은: **v1 확실한 우세 / baseline 확실한 우세 / 무승부(그 외 전부)** 세 가지이고,
"조금 나은 것 같다" 수준은 전부 무승부로 처리한다.

### 6) 반영 또는 유지

- **v1 확실한 우세일 때만** → 사용자에게 변경 diff 와 **무엇이 결정적 우위였는지 근거**를
  제시하고 승인받은 뒤 반영:
  ```
...
`fork --from v1 --name v1b` 처럼 이어간다. **확실한 최종 승자만** promote 하고, 끝까지
확실한 우위가 없으면 아무것도 promote 하지 않는다(무승부 → baseline 유지).
